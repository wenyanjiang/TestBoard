...
void BSP_VoltAndSpiSetting(uint16_t mode1,uint8_t buf,uint8_t mode2,uint8_t spiSpeed)	
{
	...
	uint32_t u32dataBuf[5];
	u32dataBuf[0] = 0;
	u32dataBuf[1] = (uint32_t)mode1;
	u32dataBuf[2] = (uint32_t)buf;
	u32dataBuf[3] = (uint32_t)mode2;
	u32dataBuf[4] = (uint32_t)spiSpeed;
	g_u16Vmod1 = mode1;
	g_u8Buf = buf;
	g_u8Vmod2 = mode2;
	...
	Dev_STFlash_Sector8WriteWord(VOL_SPI_SET_VAL_OFFSET,u32dataBuf,5,FLASH_SECTOR8_EFFECT_DATA_LENGTH);
	VMOD1_DISEN();
	...
	if(led_num == 1)//led1   w
	{
		...
		*/
		sConfigOC.OCMode = TIM_OCMODE_PWM1;
 		sConfigOC.Pulse = htim4.Init.Period*(led_light)/100;
...
short BSP_VMOD1Calibration(uint32_t *databuf)
{
	float R_Shunt;
	uint8_t data_buf[2];
	short cali_value,tmp,tmp1 = 1;
	
	float fCurrentLSB, fRegCurrentVal, fCalCurrentVal, fShuntVol;
	uint8_t u8RegBuf[2];
	short s16RegShuntVal , s16RegCurVal;
...
int Dev_SPI_ReceiveDataDMA(SPI_HandleTypeDef *hspi , uint8_t * receBuf , uint32_t size)
{
	...
	for(i = 0;i <= packCnt;i++)
	{
		...
		if(i == packCnt)
		{
			...
			retVal = HAL_SPI_Receive_DMA(&hspi1,receBuf,lastPackSize);

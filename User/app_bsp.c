...
/* version */
const u8 g_u8Version[4]={0,1,4,0};
...
{
	u16 u16mod1Vol;
	u8 u8buVol,u8mod2Vol,u8SPIspeed;
...
{	
	u8 u8chnlSel,u8resBuf[4];
	u32 u32curResult;
	ST_PRTCL_INFO stRespInfo;
	EN_RESP_TYPE enRlst = EN_RESP_SUCCESS;
	
	u8chnlSel = pstRcvInfo->u8CmdParaBuf[0];
	u8 u8mode = pstRcvInfo->u8CmdParaBuf[1];
...
{	
	...
	powerDelay =  pstRcvInfo->u8CmdParaBuf[1];
	/* POWERON OR POWEROFF CODE BEGAIN */
	
	enRlst = BSP_PowerControl(u8modSel,powerDelay);
...
{	
	u8 u8curLed,u8PWMled1,u8PWMled2;
	ST_PRTCL_INFO stRespInfo;
	EN_RESP_TYPE enRlst = EN_RESP_SUCCESS;
...
static void _app_Bsp_getFlashID(ST_PRTCL_INFO* pstRcvInfo)
{
	...
	SplitU32(dataBuf,u32flashID);
	prtcl_respPreproc(&stRespInfo, pstRcvInfo,EN_PACK_ID_RESP, enRlst, 4, dataBuf);
	Dev_Usb_resp(&stRespInfo);
}
/*CMD --- 5cH*/
static void _app_Bsp_eraseFlash(ST_PRTCL_INFO* pstRcvInfo)
{
	...
	u32startAddr = pstRcvInfo->u8CmdParaBuf[0];
	u32size = pstRcvInfo->u8CmdParaBuf[4];
...
static void _app_Bsp_getFwVersion(ST_PRTCL_INFO* pstRcvInfo)
{
	...
	prtcl_respPreproc(&stRespInfo, pstRcvInfo,EN_PACK_ID_RESP, enRlst, 4, (u8 *)g_u8Version);
	Dev_Usb_resp(&stRespInfo);
}
...
{	
	...
	{
		u16calCrc = CRC16((u8* )APP_BACKUP_ADDR, u32codeLength, 0);

...
__ALIGN_BEGIN uint8_t USBD_CDC_CfgHSDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  USB_DESC_TYPE_INTERFACE,    		// 接口描述符类型,= 04H        interface descriptor type = 04H
  ...
  0x02,                             // 测试设备类型,= 0DCH         Class code = 0DCH
  0x00,                             // 子类代码,= 0A0H             Subclass code = 0A0H
  0x00,                             // 协议代码,= 0B0H             Protocol code = 0B0H
...
static uint8_t  USBD_CDC_DataOut (USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if(pdev->pClassData != NULL)
  {
//printf("%x-%x",hcdc->RxBuffer,hcdc->RxLength);
...
{
  USBD_CDC_HandleTypeDef   *hcdc = (USBD_CDC_HandleTypeDef*) pdev->pClassData;
//printf("%x-%x",hcdc->RxBuffer,pbuff);
  hcdc->RxBuffer = pbuff;
